...
export default function MyLoanDetailPage() {
    ...
    // Prefill phone + amount when loan loads
    useEffect(() => {
        if (loan?.borrower?.phone && !payPhone) {
            setPayPhone(loan.borrower.phone);
        }
        if (totalOutstanding > 0 && !payAmount) {
            setPayAmount(String(totalOutstanding));
        }
    }, [loan?.borrower?.phone, totalOutstanding]); // eslint-disable-line react-hooks/exhaustive-deps
        ...
        return (
            <div className="flex h-[calc(100vh-4rem)] items-center justify-center">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-sm text-muted-foreground animate-pulse">Loading loan details...</p>
                </div>
            </div>
        );
    }
                        ...
                        <div className="py-12 text-center">
                            <p className="text-muted-foreground">
                                {loan.status === 'Pending Approval'
                                    ? `Repayment schedule will be generated upon loan approval.`
